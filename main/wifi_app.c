...
static void wifi_app_soft_ap_config(void)
{
	...
	esp_netif_ip_info_t ap_ip_info; // to save the three values of ap_ip which they (ip , gateway ,netmask)
	memset(&ap_ip_info, 0x00, sizeof(ap_ip_info)); // cleaning the ap_ip_info from any possible garbage values

	esp_netif_dhcps_stop(esp_netif_ap);					///> must call this first (stop the DHCP Service,which is responsible for assigning IP Addresses to devices connected to the Access Point
		...
		if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case WIFI_APP_MSG_LOAD_SAVED_CREDENTIALS:
					...
					{
						ESP_LOGI(TAG,"Unable to load status configuration");
					}
				...
				case WIFI_APP_MSG_STA_DISCONNECTED:
					...
					{
						ESP_LOGI(TAG,"WIFI_APP_STA_DISCONNECTED: ATTEMPT FROM HTTP SERVER");
						xEventGroupClearBits(wifi_app_event_group,WIFI_APP_CONNECTING_FROM_HTTP_SERVER_BIT);
					...
					else if (eventBits & WIFI_APP_USER_REQUESTED_STA_DISCONNECT_BIT)
					{
						...
						ESP_LOGI(TAG,"WIFI_APP_MSG_STA_DISCONNECTED: ATTEMP FAILED, CHECK WIFI ACCESS POINT AVAILABILITY");
						// Adjust the case to your need - maybe you want to keep trying to connect...
					}
