...
#include "esp_wifi.h"
#include "sys/param.h" // idk what the fuck is this for
#include <string.h>
...
/**
 * Check the g_fw_update_status and creates the fw_update_reset timer if g_fw_update_status is true
 */
static void http_server_fw_update_reset_timer(void)
{
	if(g_fw_update_status == OTA_UPDATE_SUCCESSFUL){
		ESP_LOGI(TAG,"http_server_fw_update_reset_timer: FW updated successful FW update reset timer");

		//Give the web page  a chance to receive an acknowledge back and initialize the timer
		ESP_ERROR_CHECK(esp_timer_create(&fw_update_reset_args,&fw_update_reset));
		ESP_ERROR_CHECK(esp_timer_start_once(fw_update_reset, 10000000));
	} else {
...
static esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
	...
	do
	{
		...
		if((recv_len = httpd_req_recv(req,ota_buff,MIN(content_length - content_received,sizeof(ota_buff)))) < 0)
		{
			...
			if(recv_len == HTTPD_SOCK_ERR_TIMEOUT)
			{
				...
				continue; ///> Retry receiving of timeout occured
			}
		...
		if(!is_req_body_started)
		{
			...
			if(body_start_p == NULL) {
				   ESP_LOGI(TAG, "http_server_OTA_update_handler: Invalid multipart header");
				   return ESP_FAIL;
			}
			body_start_p += 4;
			...
			if (body_part_len <= 0) {
			    ESP_LOGI(TAG, "http_server_OTA_update_handler: Invalid body part length");
			    return ESP_FAIL;
			}
			...
			if (esp_ota_write(ota_handle, body_start_p, body_part_len) != ESP_OK) {
			    ESP_LOGI(TAG, "http_server_OTA_update_handler: esp_ota_write failed");
			    esp_ota_abort(ota_handle);
			    return ESP_FAIL;
			}
			content_received += recv_len;
			firmware_written += body_part_len;
	     } else {
	    	 char *body_end_p = strstr(ota_buff, "\r\n------");
	     if (body_end_p != NULL)
	     {
	         int final_part_len = body_end_p - ota_buff;
	         if (final_part_len > 0)
	         {
	             if (esp_ota_write(ota_handle, ota_buff, final_part_len) != ESP_OK)
	             {
	                 ESP_LOGI(TAG, "http_server_OTA_update_handler: esp_ota_write failed");
	                 esp_ota_abort(ota_handle);
	                 return ESP_FAIL;
	             }
	             firmware_written += final_part_len;
	         }
	...
	ESP_LOGI(TAG, "http_server_OTA_update_handler: HTTP body received %d of %d bytes",
	         content_received, content_length);
	ESP_LOGI(TAG, "http_server_OTA_update_handler: Firmware written %d bytes",
	         firmware_written);

	if (content_received != content_length) {
	    ESP_LOGI(TAG, "http_server_OTA_update_handler: Warning! Request body may be incomplete");
	    esp_ota_abort(ota_handle);
	    return ESP_FAIL;
	}
	if (esp_ota_end(ota_handle) == ESP_OK) {
		...
		flash_successful = true;
		 ...
		 ESP_LOGI(TAG,"http_server_OTA_update_handler: esp_ota_set_boot_partition failed !!!");
		}
	} else {
		 ESP_LOGI(TAG,"http_server_OTA_update_handler: esp_ota_end failed !!!");
	}
	//we won't update the globel variables throughout the file,so send the message about the status
	if (flash_successful)
	{
	    ESP_LOGI(TAG, "HTTP_MSG_OTA_UPDATE_SUCCESSFUL");

	    httpd_resp_set_type(req, "text/plain");
	    httpd_resp_sendstr(req, "OTA update successful. Device will reboot.");
...
/**
 * OTA status handler responds with the firmware update status after the OTA is started
...
esp_err_t http_server_OTA_status_handler(httpd_req_t *req)
{
	...
	sprintf(otaJSON,"{\"ota_update_status\":%d,\"compile_time\":\"%s\",\"compile_date\":\"%s\"}",g_fw_update_status,__TIME__,__DATE__); // Printing the content as JSON type

	httpd_resp_set_type(req,"application/json");  // setting the type of content of the response
	httpd_resp_send(req,otaJSON,strlen(otaJSON)); // (Http_request,The string we want to sent ,the size in bytes)

	return ESP_OK;
}

/**
 * DHT sensor readings JSON handler response with DHT22 sensor data
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
...
static esp_err_t http_server_wifi_connect_json_handler(httpd_req_t *req)
{
	...
	if (len_ssid>1)
	{
		...
		{
			ESP_LOGI(TAG,"http_server_wifi_connect_json_handler: Found_header => my-connect-ssid: %s",ssid_str);
		}
	}
	// Get Password handler
		...
		if (len_pass>1)
		{
			...
			{
				ESP_LOGI(TAG,"http_server_wifi_connect_json_handler: Found_header => my-connect-pwd: %s",pass_str);
			}
		}
...
/**
 * wifiConnectStatus.json handler updates the connection status for the web page
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
 */
static esp_err_t http_server_wifi_connect_status_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG,"/wifiConnectionStatus requested");
...
static esp_err_t http_server_get_wifi_connect_Info_json_handler(httpd_req_t *req)
{
	...
	if(g_wifi_connect_status == HTTP_WIFI_STATUS_CONNECT_SUCCESS)
	{
		...
		const char *ssid = (const char*)wifi_data.ssid;
		...
		snprintf(ipInfoJSON, sizeof(ipInfoJSON),
...
static esp_err_t http_server_wifi_disconnect_json_handler(httpd_req_t *req)
{
    ...
    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(req, "{\"status\":\"disconnect_requested\"}");
...
static esp_err_t http_server_get_ap_ssid_time_json_handler(httpd_req_t *req)
{
...
snprintf(ssidJSON,sizeof(ssidJSON),"{\"ssid\":\"%s\"}",ssid);
httpd_resp_set_type(req,"application/json");
...
static httpd_handle_t http_server_configure(void)
{
	...
	// Create HTTP Queue handle for server monitor task
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		//register ap_ssid.json handler
		httpd_uri_t ap_ssid_json = {
				.uri = "/apSSID.json",
				.method = HTTP_GET,
				.handler = http_server_get_ap_ssid_time_json_handler,
				.user_ctx = NULL
		};
